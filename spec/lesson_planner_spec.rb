...
describe LessonPlanner do
  ...
  xdescribe '.write' do
    it 'adds a resource to plan' do
      ...
      expect(lesson_planner.plan).to eq ['Pencils']
    end
  end
  ...
  describe '.display' do
    it 'uses printer class' do
      ...
      expect(printer).to receive(:output) #lesson_planner = LessonPlanner.new
      
      lesson_planner = LessonPlanner.new(printer: printer)
      lesson_planner.display
